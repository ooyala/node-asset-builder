...
_.extend(NodeAssetBuilder.prototype, {
  ...
  _genDataUriFromImg: function() {
    ...
    _.each(files, function(file) {
      ...
      var extension = baseNameWithExt.match(/\.([^\..]*)$/)[1];
      var value = "      'image/"+extension+":" + baseName + "' : '" + getBase64(file, "data:image/"+extension+";base64,") + "',";
      output.push(value);
    });
  ...
  _minfiedPngFile: function() {
    ...
    _.each(files, function(file) {
       ...
       var baseName = baseNameWithExt.substring(0, baseNameWithExt.lastIndexOf('.'));
       var extension = baseNameWithExt.match(/\.([^\..]*)$/)[1];
       var outputFile = target + baseName + '.'+extension;
       gm(file).dither(true).bitdepth(8).colors(50).noProfile().quality(80).write(outputFile, function(err) {
         if (err) {
           var inStr = fs.createReadStream(file);
           var outStr = fs.createWriteStream(outputFile);
           inStr.pipe(outStr);
           console.log(err);
         }
